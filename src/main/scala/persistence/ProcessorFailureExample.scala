...
object ProcessorFailureExample extends App {
  class ExampleProcessor extends Processor {
    ...
    def receive = {
      ...
      case "no boom"                      => { received = "no boom" :: received; println(s"re ${received.reverse}")}
    ...
    override def preRestart(reason: Throwable, message: Option[Any]) {
      message match {
        case Some(p: Persistent) if !recoveryRunning => { println(s"msg → $message"); deleteMessage(p.sequenceNr) }// mark failing message as deleted
        case _                                       => println(s"ignoring...$message")// ignore
      }
