...
// Summing integers is in practice probably so fast that parallelization imposes more overhead than it saves.
// But simple examples like this are exactly the srot that are most helpful to consider when designing a 
// functional library. Complicated examples include all sorts of incidental structure and extraneous details
// that can confuse the initial design process. We are trying to understand the esssence of the problem
// domain and a good way to do this is to work with very small examples, factor out common concerns across these
...
object ParallelObj {
    ...
    //     def join: Unit
    // }
    // None of these methods return a meaningful value. Therefore, if we want to get any
    // information out of Runnable, it has to have some side effect, like mutating some internal
    // state that we know about. This is bad for composability - we cannot manipulate Runnable
    // objects generically, we need to always know something about their inter-behavior to get
    // any useful information out of them. Thread also has a downside that it maps directly onto
    // operating system threads, which are a scarce resource. We'd prefer to create as many local
    // parallel computations as natural for our problem, and later deal with mapping these logical 
    // computation onto actual OS threads.
	...
	object Par {
	  ...
	  */
      ...
      def product[A,B](fa: Par[A], fb: Par[B]) : Par[(A,B)] = map2(fa, fb)( (a,b) => (a,b) ) 
	  ...
	  */
      ...
      // I'm not using too many features of 'Map' 
      def choiceMap[A,B](a: Par[A])(choices: Map[A,Par[B]]) : Par[B] = es => choices(a(es).get)(es)
...
// it important in FP?
// In functional programming, it is easy and expected that we will factor out common
// functionality into generic, reusable, components that can
// be composed. Side effects hurt composability but more generally
// any hidden or out-of-band assumptions or behavior that prevents us
// from treating our components (be they functions or anything else) as
